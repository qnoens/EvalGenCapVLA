...
def lerobot_clean_actions_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
...
def lerobot_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # make gripper action absolute action
    abs_gripper_action = trajectory["action"][:, -1:] + trajectory["observation"]["gripper_state"]

    # make the gripper action binary
    binarized_gripper_action = binarize_gripper_actions_modified(abs_gripper_action)

    # concatenate action again
    trajectory["action"] = tf.concat(
        (
            trajectory["action"][:, :6],
            binarized_gripper_action,
        ),
        axis=-1,
    )
...
# This helper function binarizes the gripper actions from the Robotiq2F85 gripper: between 0.0 and 0.085
def binarize_gripper_actions_modified(actions: tf.Tensor) -> tf.Tensor:
    open_mask, closed_mask = actions > 0.07, actions < 0.05 # The closed mask is highly dependent on the size of the object that needs to be grasped
    ...
    return tf.scan(scan_fn, tf.range(tf.shape(actions)[0]), is_open_float[-1], reverse=True)


def pushbutton_mujoco_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    return trajectory
...
OXE_STANDARDIZATION_TRANSFORMS = {
    "pushbutton_mujoco_rlds": pushbutton_mujoco_dataset_transform,
    "pushbutton_mujoco_rlds_hard": pushbutton_mujoco_dataset_transform,
    "lerobot_rlds": lerobot_dataset_transform,
    "lerobot_rlds_clean_actions": lerobot_clean_actions_dataset_transform,
    "blue_only": lerobot_clean_actions_dataset_transform,
